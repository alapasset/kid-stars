...
export default defineEventHandler(async (event) => {
  ...
  const { data, error } = await client.from(`family`).select(`name, family_member(*)`).eq(`id`, event.context.params.id).maybeSingle()
  if (error) throw createError(error.message)
  if (!data) throw createError({statusCode: 400, statusMessage: `No family for this user`})
  return data
})
