...
import { minusPointValues, plusPointValues, type PointForm } from '~/types/point'
import { useAddPoint, useGetPoints } from '~/composables/point'
...
const { data: point, refetch } = useGetPoints(member.value.id, openDialog)
const { isPending, mutateAsync } = useAddPoint(member.value.id)
const { t } = useI18n()

const pointDialog = ref<HTMLDialogElement>()
const initialValue = computed(() => point.value?.at(0)?.sum ?? 0)

const { handleSubmit, resetForm } = useForm<PointForm>({
  initialValues: {
    points: 0,
  },
})

const { errorMessage: errorMessagePoints, setValue, value: points } = useField<number>(
  'points',
  (value) => {
      ...
      <div class="modal-box flex flex-col gap-5">
        ...
        <div class="flex flex-col items-center gap-4">
          <div class="flex gap-2">
            <button
              v-for="value in plusPointValues"
              :key="value"
          ...
          <div class="flex gap-2">
            <button
              v-for="value in minusPointValues"
              :key="value"
